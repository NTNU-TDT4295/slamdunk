...
int main(void)
{
	...
	while (1) {
		...
		performI2CRead(BNO055_EULER_H_LSB_ADDR, euler_buf, 2);
		x = ((int16_t) euler_buf[0]) | (((int16_t) euler_buf[1]) << 8);

		/* performI2CRead(BNO055_EULER_R_LSB_ADDR, euler_buf + 2, 2); */
		/* y = ((int16_t) euler_buf[2]) | (((int16_t) euler_buf[3]) << 8); */

		/* performI2CRead(BNO055_EULER_P_LSB_ADDR, euler_buf + 4, 2); */
